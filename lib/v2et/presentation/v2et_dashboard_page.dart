...
class V2etDashboardPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final offers =
        ref.watch(v2etStoreOffersProvider).valueOrNull ??
    ...
    final remoteInfo = activeProfile is RemoteProfileEntity
        ? activeProfile.subInfo
        : null;
    final totalBytes = remoteInfo?.total ?? sub?.transferEnableBytes;
    final usedBytes = remoteInfo?.consumption ?? 0;
    final remainingBytes = totalBytes == null
        ? null
    ...
    final warnTrafficBytes =
        runtimeConfig?.trafficWarnBytes ?? (3 * 1024 * 1024 * 1024);
    final expired =
        (sub?.expiredAt?.isBefore(DateTime.now()) ?? false) ||
        ((remoteInfo?.remaining.inSeconds ?? 1) <= 0);
    final outOfTraffic =
        totalBytes != null && totalBytes > 0 && usedBytes >= totalBytes;
    final warnExpirySoon =
        !expired &&
        remainingDays != null &&
        remainingDays >= 0 &&
        remainingDays <= warnDays;
    final warnTrafficSoon =
        !outOfTraffic &&
        remainingBytes != null &&
        remainingBytes > 0 &&
        remainingBytes <= warnTrafficBytes;
    final guard = expired
        ? _UsageGuard.expired
        : outOfTraffic
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          showV2etNotice(
            ...
            guard == _UsageGuard.expired
                ...
                : tr(
                    '流量已耗尽，请重置流量后使用',
                    'Traffic exhausted, reset traffic to continue',
                  ),
            error: true,
          );
        });
            ...
            return SingleChildScrollView(
              ...
              child: ConstrainedBox(
                constraints: BoxConstraints(
                  minHeight: viewport.maxHeight - (compact ? 20 : 28),
                ),
                child: Center(
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      ...
                      if (guard != _UsageGuard.ok) ...[
                        ...
                        Text(
                          warnExpirySoon
                              ...
                              : tr(
                                  '剩余流量不足，请及时重置或续费',
                                  'Low remaining traffic, please reset or renew',
                                ),
                          style: const TextStyle(
                      ...
                      ConstrainedBox(
                        ...
                        child: _Card(
                          onTap: () async {
                            final meta = await _readNodeMeta(
                              ref,
                              activeProfile,
                            );
                            var tags = meta.tags;
                            ...
                            if (tags.isEmpty && guard == _UsageGuard.ok) {
                              ...
                              final refreshedMeta = await _readNodeMeta(
                                ref,
                                activeProfile,
                              );
                                ...
                                return Dialog(
                                  ...
                                  child: Consumer(
                                    builder: (context, sheetRef, _) {
                                      final group = sheetRef
                                          .watch(
                                            proxiesOverviewNotifierProvider,
                                          )
                                          .valueOrNull;
                                      final modalLink = <String, int?>{
                                        ...linkOverrides.value,
                                      };
                                      final modalLinkLoading = <String>{
                                        ...linkLoading.value,
                                      };
  ...
  Future<_NodeMeta> _readNodeMeta(
    WidgetRef ref,
    ...
    return result.match(
      (_) => const _NodeMeta(tags: [], targets: {}),
      _extractNodeMeta,
    );
  }
  ...
  Future<void> _openRenewDialog(BuildContext context) async {
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text(
          zh ? '请续费后再使用连接功能。' : 'Please renew to continue using connection.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(dialogContext).pop(),
  ...
  Future<void> _openResetDialog(
    ...
    if (currentSession == null || !currentSession.hasToken) {
      if (context.mounted)
        showV2etNotice(
          context,
          zh ? '请先登录账号' : 'Please login first',
          error: true,
        );
      return;
    }

    final resetPlans = offers
        .where((o) => (o.prices['reset'] ?? 0) >= 0)
        .toList();
    if (resetPlans.isEmpty) {
      if (context.mounted) {
        showV2etNotice(
          context,
          zh ? '当前暂无可重置流量套餐' : 'No reset package available',
          error: true,
        );
      }
      return;
    ...
    try {
      ...
      if (methods.isEmpty) {
        if (context.mounted) {
          showV2etNotice(
            context,
            zh ? '暂无可用支付方式' : 'No payment method available',
            error: true,
          );
        }
        return;
      ...
      if (uri != null) {
        ...
        if (context.mounted) {
          showV2etNotice(
            ...
            zh
                ? '已打开支付页面，完成后可返回客户端'
                : 'Payment page opened, return after completion',
          );
        }
      }
    } catch (e) {
      if (context.mounted)
        showV2etNotice(
          context,
          (zh ? '下单失败: ' : 'Checkout failed: ') + e.toString(),
          error: true,
        );
    }
  }
...
class _LatencyAction extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final timedOut =
        hasValue && (valueMs == null || valueMs == 65535 || valueMs! <= 0);
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          Positioned.fill(
            child: IgnorePointer(
              child: CustomPaint(painter: _WorldMapSketchPainter()),
            ),
          ),
          button,
        ],
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(
                  color: active
                      ? const Color(0x555A3D89)
                      : const Color(0x2A3B2A53),
