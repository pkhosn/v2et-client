...
class V2etDashboardPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final used = activeProfile is RemoteProfileEntity ? activeProfile.subInfo?.consumption : null;
    final total = activeProfile is RemoteProfileEntity ? activeProfile.subInfo?.total : null;
    final ratio = (used != null && total != null && total > 0) ? (used / total).clamp(0.0, 1.0) : 0.0;
      ...
      appBar: AppBar(
        ...
        actions: [
          if (savedCredentials == null)
            TextButton.icon(
              onPressed: () async {
                await showDialog<void>(context: context, builder: (_) => const V2etQuickImportDialog());
              },
              icon: const Icon(Icons.login_rounded),
              label: Text(tr('登录', 'Login')),
            ),
          IconButton(
        ...
        children: [
          _Card(
            child: Column(
              ...
              children: [
                Text(savedCredentials?.email ?? tr('未登录', 'Not logged in'), style: theme.textTheme.titleLarge),
                const Gap(8),
                ...
                const Gap(4),
                Text('${tr('总流量', 'Total')}: ${_bytes(sub?.transferEnableBytes)}'),
                const Gap(4),
          ...
          _Card(
            child: Row(
              ...
              children: [
                _SpeedItem(label: tr('上传', 'Upload'), value: (stats?.uplink.toInt() ?? 0).speed()),
                _SpeedItem(label: tr('下载', 'Download'), value: (stats?.downlink.toInt() ?? 0).speed()),
              ],
            ),
          ),
          const Gap(20),
          Row(
            ...
            children: [
              ...
              const Gap(8),
              _ModeChip(label: 'TUN', selected: serviceMode == ServiceMode.tun, onTap: toggleTun),
            ],
          ),
        ],
      ),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(18),
          color: selected ? theme.colorScheme.primary : theme.colorScheme.secondaryContainer,
        ),
        child: Text(label, style: TextStyle(color: selected ? theme.colorScheme.onPrimary : null)),
      ),
    );
  }
}
...
class _SpeedItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [Text(label), const SizedBox(height: 4), Text(value, style: Theme.of(context).textTheme.titleMedium)],
    );
  }
}
