                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"V2ET"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"V2ETMutex");
  HWND handle = FindWindowA(NULL, "V2ET");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"V2ET"))
    {
      return false;
    }
  ...
  if (!window.Create(L"V2ET", origin, size))
  {
    return EXIT_FAILURE;
  }
